...
class ktc_toolchanger:
    ...
    def __init__(self, config):
        ...
        self.log = self.printer.load_object(config, 'ktc_log')
        ...
        # Register commands
        handlers = [
            'KTC_SAVE_CURRENT_TOOL', 'KTC_TOOL_LOCK', 'KTC_TOOL_UNLOCK',
            'KTC_TOOL_DROPOFF_ALL', 'KTC_SET_AND_SAVE_PARTFAN_SPEED', 'KTC_TEMPERATURE_WAIT_WITH_TOLERANCE', 
            'KTC_SET_TOOL_TEMPERATURE', 'KTC_SET_GLOBAL_OFFSET', 'KTC_SET_TOOL_OFFSET',
            'KTC_SET_PURGE_ON_TOOLCHANGE', 'KTC_SAVE_POSITION', 'KTC_SAVE_CURRENT_POSITION', 
            'KTC_RESTORE_POSITION', 'KTC_SET_GCODE_OFFSET_FOR_CURRENT_TOOL',
            'KTC_DISPLAY_TOOL_MAP', 'KTC_REMAP_TOOL', 'KTC_ENDSTOP_QUERY',
    ...
    def handle_ready(self):
        ...
        self.tool_map = self.printer.lookup_object('save_variables').allVariables.get(VARS_KTC_TOOL_MAP, {})
    ...
    def Initialize_Tool_Lock(self):
        ...
        if str(self.tool_current) == "-1":
            ...
            self.log.always("KTC initialized with T%s." % self.tool_current) 
    ...
    def ToolLock(self, ignore_locked = False):
        ...
        if not ignore_locked and int(self.tool_current) != -1:
            self.log.always("KTC_TOOL_LOCK is already locked with tool " + self.tool_current + ".")
        else:
