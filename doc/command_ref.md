...
# KTCC - Command Reference
  ...
  | Command | Description | 
  | ------- | ----------- |
  | `KTC_TOOLCHANGER_ENGAGE` | Engage or lock the tool.<br> * Toolchanger.state must be at least INITIALIZING.<br> - Runs 'engage_gcode:' if it is specified. Otherwise will set toolchanger.status to ENGAGED. |
  | `KTC_TOOLCHANGER_DISENGAGE` | Disengage or unlock from the tool. |
  | `KTC_Tn` | Activates heater, pick up and readyy the tool. If tool is mapped to another one then that tool will be selected instead. |
  | `KTC_DROPOFF` | Unloads and parks the current tool without picking up another tool, leaving the toolhead free and unlocked. Actual active extruder eill still be last used one as Klipper needs an active extruder. |  |
  ...
  ## ![#f03c15](/doc/f03c15.png) ![#c5f015](/doc/c5f015.png) ![#1589F0](/doc/1589F0.png) Debuging
  | Command | Description | Parameters |
  | ------- | ----------- | ---------- |
